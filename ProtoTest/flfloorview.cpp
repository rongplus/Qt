...
void FLFloorView::ShowContextMenu(const QPoint &pos)
{
	...
	QAction action1(QIcon(":/pro/Resources/icons/hand-icon.png"), "Moveable", this);
	connect(&action1, SIGNAL(triggered()), this, SLOT(toggleMoveable()));
	contextMenu.addAction(&action1);

	QAction action2(QIcon(":/pro/Resources/icons/add-floor-icon.png"), "Change floor plan", this);
	connect(&action2, SIGNAL(triggered()), this, SLOT(changeFloorPlan()));
	contextMenu.addAction(&action2);

	QAction action3(QIcon(":/pro/Resources/icons/add-floor-icon.png"), "Fit window", this);
	...
	if (lt.count() > 1)
	{
		QAction *action32 = new QAction(QIcon(":/pro/Resources/icons/firmware_update.png"), "Align Horizontal", this);
		contextMenu.addAction(action32);
		action32 = new QAction(QIcon(":/pro/Resources/icons/firmware_update.png"), "Align Vertical", this);
		contextMenu.addAction(action32);
		action32 = new QAction(QIcon(":/pro/Resources/icons/firmware_update.png"), "Grid", this);
		contextMenu.addAction(action32);
		action32 = new QAction(QIcon(":/pro/Resources/icons/firmware_update.png"), "Align Bottom", this);
		contextMenu.addAction(action32);
	}
	else
	{
		...
		{
			QAction *action32 = new QAction(QIcon(":/pro/Resources/icons/firmware_update.png"), "config", obj);
			connect(action32, SIGNAL(triggered()), this, SLOT(on_write_zc()));
			contextMenu.addAction(action32);

			QAction *action33 = new QAction(QIcon(":/pro/Resources/icons/firmware_update.png"), "Select all WIOs", obj);
...
void FLFloorView::wheelEvent(QWheelEvent* e)
{
	...
	if (e->modifiers() & Qt::ControlModifier) {
		...
		qreal factor = qPow(1.25, numSteps);
...
void FLFloorView::AddZC(FLZoneControllor* pZC,int nIndex)
{
	...
	QGraphicsPixmapItem *pixmap_item = new FLDeviceItem(QPixmap(":/pro/Resources/icons/zc.png"));
...
{
	QGraphicsPixmapItem *pixmap_item = new FLDeviceItem(QPixmap(":/pro/Resources/icons/wio.png"));
	pixmap_item->setParentItem(m_pFloorItem);
	//scene()->addItem(pixmap_item);
	pixmap_item->setFlag(QGraphicsItem::ItemIsMovable, true);
	pixmap_item->setFlag(QGraphicsItem::ItemIsSelectable, true);
...
void FLFloorView::link_wio(QString src, int num1, QString dest, int num2)
{
	...
	if (wio_src == nullptr || wio_dest == nullptr)
		return;
	Edge * p = nullptr;
	foreach (auto edge , list_edge_items)
	{

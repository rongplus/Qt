...
BSTree<vector<int>>* tree = NULL;
void doTreeJob(int total)
{
	...
	while(current_num < total)
	{
		...
		if( tree->search(lt) == NULL)
		{
			tree->insert(lt);
			current_num++;
		}
		else
			cout << '\r' << " find repreat =" << repeat_num++ <<  std::flush;;
	}
}
...
*/
	...
	tree=new BSTree<vector<int>>();
	thread th(printTime,nTotal);
	thread th1(doTreeJob,nTotal);
	th.join();
	th1.join();
...
cout << "total = " << letter_list.size() << "Repeat = " << repeat_num << endl;
